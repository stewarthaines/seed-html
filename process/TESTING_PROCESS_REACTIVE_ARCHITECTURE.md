...
describe('ContentService', () => {
  ...
  test('transforms plain text to XHTML using transform context', async () => {
    const result = await service.transformContent('Hello **world**!', {
...
describe('AppState Reactive Behavior', () => {
  ...
  test('coordinates services reactively without services calling each other', () => {
    cleanup = $effect.root(() => {
      ...
      const appState = new AppState(mockWorkspaceService, mockContentService, mockSettingsService);
  ...
  function createStoryAppState(workspace = null) {
    ...
    const mockContentService = {
      ...
      generateNavigationFromSpine: () =>
        Promise.resolve({
          xhtmlContent: '<nav>Mock nav</nav>',
          metadata: { id: 'nav', properties: ['nav'] },
        }),
    };
  ...
  {#snippet template(args)}
    {(() => {
      const appState = createStoryAppState({
        id: 'empty-workspace',
        opf: {
          metadata: { title: 'Empty Book', language: 'en', identifier: 'empty-123' },
          manifest: [],
          spine: [],
        },
        pathInfo: { rootfilePath: 'OEBPS/content.opf', basePath: 'OEBPS' },
...
<Story name="Workspace Loading Cycle">
  {#snippet template()}
    ...
    <div>
      ...
      <button onclick={() => appState.setWorkspaceId('demo-workspace')}> Load Workspace </button>
      <WorkspaceView />
    </div>
  {/snippet}
</Story>
...
<Story name="Service Integration Demo">
  {#snippet template()}
    ...
    <div>
      ...
      <button onclick={() => appState.setWorkspaceId('backend-demo')}> Load from Backend </button>
    </div>
  {/snippet}
</Story>
```

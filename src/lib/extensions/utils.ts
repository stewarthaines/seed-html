...
export function validateExtensionFile(file: File): ValidationResult {
  ...
  if (filename === 'license.txt' || filename === 'license') {
    if (file.size > 1024 * 1024) { // 1MB limit for license
      return {
        isValid: false,
        fileType: 'license',
 ...
 */
export function createExtensionSignature(files: Array<{ name: string; size: number }>): ExtensionSignature {
 ...
 */
export function compareExtensionSignatures(sig1: ExtensionSignature, sig2: ExtensionSignature): boolean {
  if (sig1.totalSize !== sig2.totalSize) return false;
  if (sig1.files.length !== sig2.files.length) return false;
  
  return sig1.files.every((file, index) => 
    file.name === sig2.files[index].name && file.size === sig2.files[index].size
  );
}

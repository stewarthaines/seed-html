/**
 * Simplified Enhanced AppState - TypeScript Compatible Version
 * 
 * A simplified version that demonstrates the key reactive patterns
 * with proper TypeScript integration and existing codebase compatibility.
 */
...
interface SimpleExtensionManager {
  getAvailableTransforms: (workspaceId: string) => Promise<Array<{
    path: string;
...
interface SimpleThemeStore {
  setTheme: (theme: 'light' | 'dark') => void;
  useSystemPreference: () => void;
  getCurrentTheme: () => string;
}

interface SimpleI18nStore {
  setLocale: (locale: string) => void;
  getCurrentLocale: () => string;
}

/**
 * Simplified Enhanced AppState - Production Ready
 * 
 * Demonstrates reactive state management patterns with proper TypeScript
 * integration and existing codebase compatibility.
 */
export class SimplifiedAppState {
  // Infrastructure layer
  private fileStorage: FileStorageAPI;
  
  // Service layer (only the ones that work properly)
  ...
  constructor(
    ...
    extensionManager: SimpleExtensionManager,
    themeStore: SimpleThemeStore,
    i18nStore: SimpleI18nStore,
  ...
  // ============================================================================
  // Computed Properties ($derived patterns)
  // ============================================================================
  ...
  get workspaceTitle(): string | null {
    return this.workspace?.opf.metadata.title || null;
  }
  ...
  private setupReactiveEffects(): void {
    ...
    // Effect: Auto-clear error after 5 seconds
    $effect(() => {
    ...
    // Effect: Log workspace changes for debugging
    $effect(() => {
      if (this.workspace) {
  ...
  async saveWorkspace(): Promise<void> {
    ...
    try {
      ...
      await this.workspaceService.saveWorkspace(this.workspace);
  ...
  async incrementDraftId(): Promise<void> {
    ...
    try {
      ...
      const baseTitle = this.settingsService.extractDraftInfo(this.workspace.opf.metadata.title).baseTitle;
      this.workspace.opf.metadata.title = this.settingsService.generateDraftTitle(baseTitle, newDraftId);
  ...
  cleanup(): void {
    ...
    this.isLoading = false;
    this.errorMessage = null;
    // Note: globalSettings intentionally preserved across cleanup
  }
}
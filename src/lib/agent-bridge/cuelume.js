/* eslint-disable -- vendored third-party asset, not app code */
// @ts-nocheck -- compiled output of a TypeScript package, vendored without the
// .d.ts files that gave it types; checkJs would otherwise report 31 errors in
// code this repo does not maintain.
/**
 * Cuelume 0.2.1 — curated interaction sounds synthesized via the Web Audio API.
 * MIT, Copyright (c) 2026 Daniel Belyi. Licence text: cuelume-LICENSE.txt.
 * Upstream: https://github.com/Danilaa1/cuelume
 *
 * Vendored rather than depended on, because the agent bridge module is served
 * RAW by dev middleware (vite.config.ts) — no bundler, so no bare-specifier
 * resolution — and because everything here must stay out of the production
 * build. Kept dev-only beside module.js instead of in public/, which ships.
 *
 * Assembled from the package's dist/sounds/recipes.js and dist/audio/engine.js,
 * concatenated in that order with the cross-file import dropped. The published
 * dist/index.js and dist/interactions/bind.js are omitted: bind() wires
 * data-cuelume-* attributes across a document, and the bridge overlay calls
 * play() directly for a handful of named cues.
 *
 * Upstream formatting is deliberately untouched (see .prettierignore) so this
 * stays diffable against the package. Exports: play, setEnabled, setVolume,
 * sounds, isSoundName, RECIPES.
 */

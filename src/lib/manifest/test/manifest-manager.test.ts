...
import { ManifestManagerImpl, ManifestValidator } from '../index.js';
...
describe('ManifestManager', () => {
  ...
  describe('loadManifest', () => {
    ...
    it('should throw WorkspaceNotFoundError for invalid workspace', async () => {
      ...
      await expect(manifestManager.loadManifest('invalid-id')).rejects.toThrow(
        'Workspace not found'
      );
    });
    ...
    it('should throw ManifestCorruptedError for corrupted OPF data', async () => {
      ...
      await expect(manifestManager.loadManifest(testWorkspaceId)).rejects.toThrow(
        'Manifest corrupted'
      );
    });
  });
  ...
  describe('getManifestItem', () => {
    ...
    it('should throw ItemNotFoundError for non-existent item', async () => {
      ...
      await expect(
        manifestManager.getManifestItem(testWorkspaceId, 'missing-item')
  ...
  describe('updateManifestItem', () => {
    ...
    it('should throw ItemNotFoundError for invalid item ID', async () => {
      ...
      await expect(
        manifestManager.updateManifestItem(testWorkspaceId, 'missing-item', {
  ...
  describe('deleteManifestItem', () => {
    ...
    it('should throw ItemNotFoundError for non-existent item', async () => {
      ...
      await expect(
        manifestManager.deleteManifestItem(testWorkspaceId, 'missing-item')
      ).rejects.toThrow('Item not found');
    });
  ...
  describe('setItemContent', () => {
    it('should save text content successfully', async () => {
      ...
      const savedContent = await mockWorkspaceManager.readTextFile(
        testWorkspaceId,
        'OEBPS/text.html'
      );
    ...
    it('should throw StorageQuotaExceededError when storage limit reached', async () => {
      ...
      await expect(
        manifestManager.setItemContent(testWorkspaceId, 'text-item', 'content')
  ...
  describe('createTextItem', () => {
    ...
    it('should throw DuplicateItemError for existing ID', async () => {
      ...
      await expect(manifestManager.createTextItem(testWorkspaceId, itemData)).rejects.toThrow(
        'Duplicate item'
      );
    });
  ...
  describe('createFileItem', () => {
    ...
    it('should use custom target path when provided', async () => {
      ...
      const result = await manifestManager.createFileItem(
        testWorkspaceId,
        mockFile as File,
    ...
    it('should throw ContentTooBigError for oversized files', async () => {
      ...
      await expect(
        manifestManager.createFileItem(testWorkspaceId, mockFile as File)
  ...
  describe('Error Handling', () => {
    ...
    it('should propagate OPF write failures', async () => {
      ...
      await expect(
        manifestManager.updateManifestItem(testWorkspaceId, 'item1', {
          mediaType: 'application/xhtml+xml',
        })

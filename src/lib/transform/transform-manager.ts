...
export class TransformManager {
  ...
  async loadTransformSettings(workspaceId: string): Promise<TransformSettings> {
    try {
      ...
      try {
        ...
        throw new Error(`Invalid JSON in settings file: ${parseError instanceof Error ? parseError.message : 'Unknown parsing error'}`);
      }
    } catch (error) {
  ...
  validateTransformScript(
    ...
    try {
      ...
      result.isValid = false;
      result.errors.push(`Syntax error: ${syntaxError instanceof Error ? syntaxError.message : 'Unknown syntax error'}`);
      return result;
    }
    ...
    const dangerousPatterns = [
      ...
      { pattern: /Function\s*\(/g, warning: 'Use of Function() constructor is potentially dangerous' },

...
export const SAMPLE_EXTENSIONS: Record<string, SampleExtension> = {
  ...
  PRISM: {
    ...
    files: {
      ...
      'prism-javascript.js':
        '// JavaScript language support for Prism\nPrism.languages.javascript = {/* ... */};',
    },
  ...
  KATEX: {
    ...
    files: {
      ...
      'katex.min.css':
        '/* KaTeX CSS styles for math rendering */.katex{font:1.21em KaTeX_Main,Times New Roman,serif}',
  ...
  ABCJS_COMPLETE: {
    ...
    files: {
      'abcjs-basic.min.js': createMinifiedJs('abcjs', '6.2.2', 145),
      'abcjs-plugin.js':
        '// Extended functionality for abcjs\nabcjs.renderAbc = function() {/* ... */};',
...
export const EXTENSIONS_BY_CATEGORY = {
  ...
  'Syntax Highlighting': [
    ...
    SAMPLE_EXTENSIONS.HIGHLIGHT_COMPLETE,
    SAMPLE_EXTENSIONS.PRISM,
  ],
  'Data Visualization': [SAMPLE_EXTENSIONS.D3, SAMPLE_EXTENSIONS.CHART_JS],
  Math: [SAMPLE_EXTENSIONS.KATEX, SAMPLE_EXTENSIONS.MATHJAX],
  'Music Notation': [
    SAMPLE_EXTENSIONS.ABCJS,
    SAMPLE_EXTENSIONS.ABCJS_COMPLETE,
    SAMPLE_EXTENSIONS.VEXFLOW,
  ],
...
export function createSampleFiles(extension: SampleExtension): File[] {
  return Object.keys(extension.files).map(filename => createSampleFile(extension, filename));
}

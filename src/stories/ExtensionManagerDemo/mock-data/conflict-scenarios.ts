...
export const CONFLICT_SCENARIOS: Record<string, ConflictScenario> = {
  VERSION_MISMATCH: {
    ...
    workspaceExtension: {
      ...
      files: {
        'markdown-it.min.js':
          '/*! markdown-it v12.3.2 */ function(){var a,b;'.repeat(250) + '})();',
      },
  ...
  CONTENT_MODIFIED: {
    ...
    workspaceExtension: {
      ...
      files: {
        'lodash.min.js':
          '/*! Lodash.js v4.17.21 CUSTOM BUILD */ function(){var custom;'.repeat(400) + '})();',
      },
  ...
  PARTIAL_OVERLAP: {
    ...
    workspaceExtension: {
      ...
      files: {
        ...
        'abcjs-plugin.js':
          '// Different plugin implementation\nabcjs.customRender = function() {};',
        'custom-styles.css':
          '/* Additional CSS styles for ABC rendering */.abc-notation { color: blue; }',
...
export function getSuggestedResolution(
  conflictType: ConflictScenario['conflictType']
): ConflictResolution[] {
  switch (conflictType) {
    case 'version':
      return [
        ConflictResolution.KEEP_CACHE,
        ConflictResolution.KEEP_WORKSPACE,
        ConflictResolution.RENAME_EXTENSION,
      ];
    case 'content':
      return [
        ConflictResolution.KEEP_WORKSPACE,
        ConflictResolution.SKIP_CACHING,
        ConflictResolution.RENAME_EXTENSION,
      ];
    case 'files':
      return [
        ConflictResolution.MERGE_FILES,
        ConflictResolution.KEEP_WORKSPACE,
        ConflictResolution.KEEP_CACHE,
      ];
    default:
 ...
 */
export function generatePerformanceExtensions(
  scenario: typeof PERFORMANCE_SCENARIOS.MANY_SMALL_EXTENSIONS
  ...
  for (let i = 0; i < scenario.count; i++) {
    ...
    const jsContent =
      `/*! Performance Test Library ${i} */\nfunction perfLib${i}() {\n`.repeat(
        scenario.avgSize / 100
      ) + '})();';

    workspaceFiles[`SOURCE/extensions/${extensionName}/${extensionName}.min.js`] = jsContent;
    workspaceFiles[`SOURCE/extensions/${extensionName}/LICENSE.txt`] =
      `MIT License\n\nPerformance Test Library ${i}`;
  }

<script lang="ts">
	...
	import { BASIC_EPUB_WORKSPACE, MockFileStorageAPI } from './mock-data';
	...
	async function handlePackage() {
		...
		try {
			const result = await packager.packageEPUB(BASIC_EPUB_WORKSPACE.id, {
				progressCallback: showProgress ? (p) => {
					progress = p;
				} : undefined
			});
		...
		<div class="results">
			...
			{#if packageResult.success}
				<div class="success">
					...
					<div class="result-item">
						<strong>Files:</strong> {packageResult.fileCount}
					</div>
					<div class="result-item">
						<strong>Original Size:</strong> {formatBytes(packageResult.totalSize || 0)}
					</div>
					<div class="result-item">
						<strong>Compressed Size:</strong> {formatBytes(packageResult.compressedSize || 0)}
					</div>
					<div class="result-item">
						<strong>Compression Ratio:</strong> 
						{Math.round(((packageResult.totalSize || 0) - (packageResult.compressedSize || 0)) / (packageResult.totalSize || 1) * 100)}%
					</div>
					<div class="result-item">
						<strong>Processing Time:</strong> {packageResult.processingTime}ms
					</div>

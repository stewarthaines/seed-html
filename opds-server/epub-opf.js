...
function extractMetadataFromPackage(packageData) {
  ...
  const identifier = identifierElements.length > 0 
    ? identifierElements[0]._ || identifierElements[0]
    : null;
  ...
  if (!dctermsModified.endsWith('Z')) {
    throw new Error(`Invalid dcterms:modified format: ${dctermsModified} (must be UTC with Z suffix)`);
  }
  ...
  const creator = creatorElements.length > 0 
  ...
  const description = descriptionElements.length > 0 
    ? descriptionElements[0]._ || descriptionElements[0] || ''
    : '';
  
  const languageElements = metadata['dc:language'] || [];
  const language = languageElements.length > 0 
    ? languageElements[0]._ || languageElements[0] || 'en'
    : 'en';

...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Validation and accessibility findings for the open project, each tagged with a triage category — 'fixable' (remedy in author-editable files; the tagged remedy surface says where), 'explainable' (real issue, remedy outside the sources — explain and propose), 'app-bug' (SEED's own generated output — tell the author to report it, do not 'fix' the book), 'not-applicable' (web-app rule noise). epubcheck: the author's last package+validate report with an explicit freshness status — none | different-project | stale | current; 'stale' means the project was edited after validation, so ask the author to package + validate before trusting details. a11y (axe) is not served yet and reports unavailable.",
    inputSchema: { type: 'object', properties: {} },
  },
  {

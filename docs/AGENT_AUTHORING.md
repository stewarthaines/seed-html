...
- **Reading systems clamp images with inline `!important` sizing, computed from a snapshot.** Foliate-class engines (SEED's device previews included) stamp every `img` at layout init: author `max-height` is overwritten with the page height (height-driven sizing dies entirely there), and `max-width` is preserved only when it computes FINITE — `none` becomes `100%`, which under a `min-content` wrapper is circular and collapses the figure to a sliver. The snapshot also races stylesheet load, so a stylesheet-carried cap can lose intermittently: put the finite cap INLINE on the img (a DOM transform's job). The HTML `width` attribute is the one per-image sizing input the clamps never touch — the most portable dial there is. To keep a caption at the image's width inside a shrink-wrapped float, give the figcaption `width: 0; min-width: 100%` so it contributes no preferred width.
...
- The PDF path is itself **three rasterizations**: the app's paged preview, the Generate-PDF tab, and the browser's print dialog ("Save as PDF"). They can disagree, and **the only authority is the saved PDF** — open it and check. Every earlier surface lies in some browser: size-contained boxes can rasterize BLANK in a print dialog that renders everything else (Firefox); Safari's print PREVIEW can show only the first pages of a long document on first invocation and fill in on a second pass, while the file it saves is complete throughout. Before diagnosing "content is missing from the PDF", save the file and look at it; when a preview and a saved file disagree, believe the file.
- **`break-*` rules need UNPREFIXED selectors.** Paged.js consumes `break-before`/`break-after`/`break-inside` declarations at stylesheet-parse time and registers them by matching selectors against the SOURCE content — before any `.pagedjs_*` wrapper exists. `.pagedjs_page_content .lyrics { break-before: page }` silently never registers (every other declaration in the rule still applies, which hides the failure); plain `.lyrics { break-before: page }` works. Non-break declarations may keep the prefix.
- **Drop size containment for print, and pick container-query-managed layouts with plain rules.** `container-type` boxes can print blank (see above), and print rendering may treat size container queries as never matching — blanking or unstyling everything they manage. The pattern (see the abc2svg and responsive extensions' `@media print` blocks): `container-type: normal` in print, plus explicit print rules for whichever variant/layout the queries would have chosen.
- **Blocks that must not fragment, must say so.** A multicol block split across a page boundary lays each fragment's columns independently — worst case off the page edge, content invisible. A float split across a column or page boundary loses its line-box exclusion — text paints straight over the image. Give both `break-inside: avoid`; a block that no longer fits then moves whole to the next page instead of corrupting.
...
- **Track changes** (`reviewMode: true` in `seed_project_info`; `settings.track_changes` in the project setup) means the app keeps a base copy of each chapter source, stylesheet and script under `SOURCE/main/<path>` the first time an edit changes it, so the author can produce a patchset later. Expect those files to appear after your writes — a write result says `baseCaptured` when yours made one — and leave them alone: they are the author's base, not chapters, not clutter.

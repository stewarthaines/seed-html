...
3. Match the project's insertion templates: `image_template` (placeholders `<href>`, `<alt>`), `video_template`, `audio_clip_template` in settings show how this project embeds media in source text. Use them; don't invent markup.
4. `SOURCE/preview/head.xml` (when configured) is authoring-time-only markup injected into previews — never part of the packaged book. Don't put production styles there.
